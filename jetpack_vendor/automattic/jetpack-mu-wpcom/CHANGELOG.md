...
- Expiry notices: Show the plan-expiry notice on the site front-end to logged-in admins.
...
- AI Launchpad: rebuild the product analytics plane — shared context props on every Tracks event, full wizard/task funnel events, server-side task/all-tasks completion reporting, analytics-only inferred_goal, and tailoring telemetry folded into the tailored Logstash record.
...
- Gutenberg: Enable the React 19 experiment on 40% of Atomic sites, and skip more incompatible plugins.
- Gutenberg: Keep the React 19 experiment disabled on sites running Gutenberg 23.9.0.
- Gutenberg: Skip the React 19 experiment only on extension versions that are still incompatible.
...
- Write: Send a prompt answer straight to the Block editor when this browser has opted out of Write, whichever surface the answer came from.
...
- Premium Analytics: Stop bundling the dashboard package, which WordPress.com now runs from the copy shipped with the Jetpack plugin.
- Premium Analytics: Stop starting the dashboard on WordPress.com Simple, which now starts it itself.
...
- Remove the transitional Subscribers announcement page from the Jetpack menu.
- Remove the WPCOM Smart Dictation editor feature.
- Remove unnecessary unminified JS.
- RTC: Remove welcome modal hook.
- Stop bundling the podcast package; Jetpack provides it on Simple and Atomic.
