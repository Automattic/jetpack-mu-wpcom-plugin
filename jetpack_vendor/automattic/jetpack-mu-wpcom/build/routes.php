...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jetpack_mu_wpcom_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jetpack_mu_wpcom_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-mu-wpcom/routes/' . $route['name'] . '/content';
				$extension = '.min.js';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-mu-wpcom/routes/' . $route['name'] . '/route';
				$extension = '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
