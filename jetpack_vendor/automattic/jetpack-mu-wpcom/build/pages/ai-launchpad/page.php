...
global $jetpack_mu_wpcom_ai_launchpad_routes, $jetpack_mu_wpcom_ai_launchpad_menu_items;
$jetpack_mu_wpcom_ai_launchpad_routes     = array();
$jetpack_mu_wpcom_ai_launchpad_menu_items = array();

/**
 * Register a route for the ai-launchpad page.
 *
 ...
 */
function jetpack_mu_wpcom_register_ai_launchpad_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_mu_wpcom_ai_launchpad_routes;
	...
	$jetpack_mu_wpcom_ai_launchpad_routes[] = $route;
}

/**
 * Register a menu item for the ai-launchpad page.
 *
 ...
 */
function jetpack_mu_wpcom_register_ai_launchpad_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $jetpack_mu_wpcom_ai_launchpad_menu_items;
	...
	$jetpack_mu_wpcom_ai_launchpad_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the ai-launchpad page.
 *
 * @return array Array of route objects.
 */
function jetpack_mu_wpcom_get_ai_launchpad_routes() {
	global $jetpack_mu_wpcom_ai_launchpad_routes;
	return $jetpack_mu_wpcom_ai_launchpad_routes ?? array();
}

/**
 * Get all registered menu items for the ai-launchpad page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_mu_wpcom_get_ai_launchpad_menu_items() {
	global $jetpack_mu_wpcom_ai_launchpad_menu_items;
	return $jetpack_mu_wpcom_ai_launchpad_menu_items ?? array();
}

/**
 * Preload REST API data for the ai-launchpad page.
 * Automatically called during page rendering.
 */
function jetpack_mu_wpcom_ai_launchpad_preload_data() {
...
/**
 * Render the ai-launchpad page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function jetpack_mu_wpcom_ai_launchpad_render_page() {
	...
	jetpack_mu_wpcom_ai_launchpad_preload_data();

	// Get all registered routes and menu items
	$menu_items = jetpack_mu_wpcom_get_ai_launchpad_menu_items();
	$routes = jetpack_mu_wpcom_get_ai_launchpad_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'ai-launchpad-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = [];
		wp_add_inline_script(
			'ai-launchpad-prerequisites',
			sprintf(
		...
		wp_register_style( 'ai-launchpad-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for ai-launchpad module
		...
		$boot_dependencies = apply_filters(
			'ai-launchpad_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_enqueue_script( 'ai-launchpad-prerequisites' );
		wp_enqueue_script_module( 'ai-launchpad' );
		wp_enqueue_style( 'ai-launchpad-prerequisites' );
	}
	...
	<body class="ai-launchpad">
		<div id="ai-launchpad-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function jetpack_mu_wpcom_ai_launchpad_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'ai-launchpad' === $_GET['page'] ) {
		jetpack_mu_wpcom_ai_launchpad_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'jetpack_mu_wpcom_ai_launchpad_intercept_render' );
