<?php
/**
 * Page: site-setup (wp-admin integrated)
...
// Global storage for site-setup routes and menu items
global $jetpack_mu_wpcom_site_setup_wp_admin_routes, $jetpack_mu_wpcom_site_setup_wp_admin_menu_items;
$jetpack_mu_wpcom_site_setup_wp_admin_routes     = array();
$jetpack_mu_wpcom_site_setup_wp_admin_menu_items = array();

/**
 * Register a route for the site-setup-wp-admin page.
 *
 ...
 */
function jetpack_mu_wpcom_register_site_setup_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_mu_wpcom_site_setup_wp_admin_routes;
	...
	$jetpack_mu_wpcom_site_setup_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the site-setup-wp-admin page.
 ...
 */
function jetpack_mu_wpcom_register_site_setup_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $jetpack_mu_wpcom_site_setup_wp_admin_menu_items;
	...
	$jetpack_mu_wpcom_site_setup_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the site-setup-wp-admin page.
 *
 * @return array Array of route objects.
 */
function jetpack_mu_wpcom_get_site_setup_wp_admin_routes() {
	global $jetpack_mu_wpcom_site_setup_wp_admin_routes;
	return $jetpack_mu_wpcom_site_setup_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the site-setup-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_mu_wpcom_get_site_setup_wp_admin_menu_items() {
	global $jetpack_mu_wpcom_site_setup_wp_admin_menu_items;
	return $jetpack_mu_wpcom_site_setup_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the site-setup-wp-admin page.
 * Automatically called during page rendering.
 */
function jetpack_mu_wpcom_site_setup_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the site-setup-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function jetpack_mu_wpcom_site_setup_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=site-setup-wp-admin (plugin)
	// 2. Direct file via site-setup.php (Core) - screen ID will be 'site-setup'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'site-setup-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'site-setup' === $current_screen->id )
	);
	...
	/**
	 * Fires when the site-setup admin page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'site-setup-wp-admin_init' );

	// Preload REST API data
	jetpack_mu_wpcom_site_setup_wp_admin_preload_data();

	// Get all registered routes
	$routes = jetpack_mu_wpcom_get_site_setup_wp_admin_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'site-setup-wp-admin-prerequisites', '', $asset['dependencies'], $asset['version'], true );
...
JS;
		wp_add_inline_script(
			...
			sprintf(
				'( %s )( %s, %s, %s );',
				$init_js_function,
				wp_json_encode( 'site-setup-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		wp_register_style( 'site-setup-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for site-setup-wp-admin module
		...
		$boot_dependencies = apply_filters(
			'site-setup-wp-admin_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_register_script_module(
			'site-setup-wp-admin',
			$build_constants['build_url'] . 'pages/site-setup/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'site-setup-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'site-setup-wp-admin' );
		wp_enqueue_style( 'site-setup-wp-admin-prerequisites' );
	}
}

/**
 * Render the site-setup-wp-admin page.
 * Call this function from add_menu_page or add_submenu_page.
 * This renders within the normal WordPress admin interface.
 */
function jetpack_mu_wpcom_site_setup_wp_admin_render_page() {
	?>
	<style>
		...
		body.js #wpbody-content > div:not(#site-setup-wp-admin-app):not(#screen-meta) {
			display: none;
		}
		body.js #wpfooter {
			display: none;
	...
	?>
	<div id="site-setup-wp-admin-app" class="boot-layout-container"></div>
	<?php
}

// Hook the enqueue function to admin_enqueue_scripts
add_action( 'admin_enqueue_scripts', 'jetpack_mu_wpcom_site_setup_wp_admin_enqueue_scripts' );
