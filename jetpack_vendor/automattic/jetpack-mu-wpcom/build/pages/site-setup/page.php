...
global $jetpack_mu_wpcom_site_setup_routes, $jetpack_mu_wpcom_site_setup_menu_items;
$jetpack_mu_wpcom_site_setup_routes     = array();
$jetpack_mu_wpcom_site_setup_menu_items = array();

/**
 * Register a route for the site-setup page.
 *
 ...
 */
function jetpack_mu_wpcom_register_site_setup_route( $path, $content_module = null, $route_module = null ) {
	global $jetpack_mu_wpcom_site_setup_routes;
	...
	$jetpack_mu_wpcom_site_setup_routes[] = $route;
}

/**
 * Register a menu item for the site-setup page.
 *
 ...
 */
function jetpack_mu_wpcom_register_site_setup_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $jetpack_mu_wpcom_site_setup_menu_items;
	...
	$jetpack_mu_wpcom_site_setup_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the site-setup page.
 *
 * @return array Array of route objects.
 */
function jetpack_mu_wpcom_get_site_setup_routes() {
	global $jetpack_mu_wpcom_site_setup_routes;
	return $jetpack_mu_wpcom_site_setup_routes ?? array();
}

/**
 * Get all registered menu items for the site-setup page.
 *
 * @return array Array of menu item objects.
 */
function jetpack_mu_wpcom_get_site_setup_menu_items() {
	global $jetpack_mu_wpcom_site_setup_menu_items;
	return $jetpack_mu_wpcom_site_setup_menu_items ?? array();
}

/**
 * Preload REST API data for the site-setup page.
 * Automatically called during page rendering.
 */
function jetpack_mu_wpcom_site_setup_preload_data() {
 ...
 */
function jetpack_mu_wpcom_site_setup_render_page() {
	...
	/**
	 * Fires when the site-setup page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'site-setup_init' );
	...
	jetpack_mu_wpcom_site_setup_preload_data();

	// Get all registered routes and menu items
	$menu_items = jetpack_mu_wpcom_get_site_setup_menu_items();
	$routes = jetpack_mu_wpcom_get_site_setup_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'site-setup-prerequisites', '', $asset['dependencies'], $asset['version'], true );
		...
		wp_register_style( 'site-setup-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for site-setup module
		...
		wp_enqueue_script( 'site-setup-prerequisites' );
		wp_enqueue_script_module( 'site-setup' );
		wp_enqueue_style( 'site-setup-prerequisites' );
	}
	...
	?>
		...
		<div id="site-setup-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function jetpack_mu_wpcom_site_setup_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'site-setup' === $_GET['page'] ) {
		jetpack_mu_wpcom_site_setup_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'jetpack_mu_wpcom_site_setup_intercept_render' );
